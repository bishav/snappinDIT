var issueType, serviceTag, triggerChatButtonId=null, showSnapin=false, lastwaitingVal=null;
(function() {
    var initESW;
...
function triggerSnapin(snapInObject) {
    initESW = function(gslbBaseURL) {
        ...
        showSnapin = snapInObject.snapinVisability;
...
//Waiting checking the value
eleExist(".dockableContainer .embeddedServiceLiveAgentStateWaiting .waitingStateContainer .waitingStateContent .header", waitingStateValue);

eleExist(".embeddedServiceSidebarFeature .chasitorInputWrapper .chasitorControls .uiInputTextArea  .chasitorText", chatStarted);
...
function waitingStateValue(eleSelector, findingEle){
    var waitingValue = $(eleSelector).text();
    if(waitingValue != lastwaitingVal){
        var d = new Date();
        activityLog(d+" Waiting shows the following message: "+waitingValue);
        lastwaitingVal = waitingValue;
    }
    
    //clearInterval(findingEle);
}

function chatStarted(eleSelector, findingEle){
    var d = new Date();
    activityLog(d+" ChatStarted");
    clearInterval(findingEle);
}
//BNR
...
function addCharectorRemaining(eleSelector, findingEle){
    if($("#snappinCharCounter").length == 0){
        ...
        sendFormPrechatVal("Prechat form is Opende");
        
        $(".dockableContainer .closeButton").click(function(){
            sendFormPrechatVal("Exit Button");
        });
        
        $(".dockableContainer .embeddedServiceLiveAgentStatePrechatDefaultUI .embeddedServiceSidebarButton").click(function(){
            sendFormPrechatVal("Start Chat Button");
        });
    }
    $("#"+triggerChatButtonId).attr("disabled", false);
...
function sendFormPrechatVal(val){
	var firstNameLog = $(".embeddedServiceLiveAgentStatePrechatDefaultUI .FirstName").val();
	var lastNameLog = $(".embeddedServiceLiveAgentStatePrechatDefaultUI .LastName").val();
	var emailLog = $(".embeddedServiceLiveAgentStatePrechatDefaultUI .Email").val();
	var primePhoneLog = $(".embeddedServiceLiveAgentStatePrechatDefaultUI .Primary_Phone__c").val();
	var issueDescLog = $(".embeddedServiceLiveAgentStatePrechatDefaultUI .Issue_Description__c").val();
	var d = new Date();
    activityLog(d+" "+val+ ": Where the values are Name: "+firstNameLog+" "+lastNameLog+"; Email: "+emailLog+"; Primery Phone:"+primePhoneLog+"; Issue Description:"+issueDescLog);
}
