...
function createCustPreChat(preChatlableObject , user, subscriptionList) {
    ...
    if (subscriptionList && subscriptionList.length > 1) {//If number of subscrition is greated than 1
        ...
        subscriptionDom = '<li class="cusPreChat-inputEmail cusPreChat-embeddedServiceSidebarFormField"> <div class="cusPreChat-uiInput cusPreChat-uiInputEmail cusPreChat-uiInput--default cusPreChat-uiInput--input"> <label class="cusPreChat-uiLabel-left cusPreChat-form-element__label cusPreChat-uiLabel" for="Email"> <span>' + preChatlableObject.subscription + '</span></label><select id="cusPreChat-subscription" class="cusPreChat-subscription form-control cusPreChat-input">' + subscriptionListDom + '</select></div></li><li class="cusPreChat-loaderDisplay" style="display:none;"><div class="cusPreChat-loader"></div></li>';
    ...
    document.getElementById("cusPreChat-nextBtn").addEventListener("click", function(){
        getButtonQueue(1, showrestOfPrechatForm, showAgentOfflneInPrechat)
...
function showrestOfPrechatForm(resultObject){
    alert(resultObject.buttonId);
    

    if(resultObject.agentAvilable == true){
        alert("Yes...");
    }
}
